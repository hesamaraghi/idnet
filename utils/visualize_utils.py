...
def animate_events(events, transform, fig_size=None):
    ...
    if is_pyg_event_data(events):
        ...
        assert is_numpy_event_data(
            events
        ), "The input data must be a structured numpy array or a PyG dataset."
...
def put_event_side_by_side(datas, image_size, axis=0):
    ...
    for i in range(1, num_data):
        ...
        data_combined.pos = torch.cat(
            [data_combined.pos, data_to_be_shifted.pos], dim=0
        )
