...
def numpy2pyg_event_convertor(events):
    ...
    data_p = (
        events["p"].reshape(-1, 1).astype(np.float32) * 2 - 1.0
...
def is_numpy_event_data(data):
    ...
    if isinstance(data, np.ndarray) and data.dtype.names is not None:
        assert (
            "x" in data.dtype.names
        ), "The structured numpy array must have a field named 'x'."
        assert (
            "y" in data.dtype.names
        ), "The structured numpy array must have a field named 'y'."
        assert (
            "t" in data.dtype.names
        ), "The structured numpy array must have a field named 't'."
        assert (
            "p" in data.dtype.names
        ), "The structured numpy array must have a field named 'p'."
        return True
    return False
