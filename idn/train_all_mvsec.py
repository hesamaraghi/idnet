...
@hydra.main(config_path="config", config_name="mvsec_train_all")
# @hydra.main(config_path="config", config_name="tid_train")
# @hydra.main(config_path="config", config_name="id_train_original_all")
...
def main(config):
    ...
    config = OmegaConf.merge(config, cmd_cfg)
    # Print fully resolved config (interpolations and references expanded)
